...
small_file = "/content/drive/MyDrive/Colab Notebooks/small1.csv"
...
support = 4
partitions = rdd2.getNumPartitions()
partitions
...
filter_val = int(sys.argv[1])
support2 = int(sys.argv[2])
...
def process_ta_feng_data(input_file, filter_val):
  df = sc.textFile(input_file)
  first_line = df.first()

  rdd1 = df.filter(lambda x: x!= first_line)

  rdd2 = rdd1.map(lambda x: ((x.split("-")[0][0:8]+"-"+x.split(",")[1]).replace('"',""), str(int(x.split(",")[5][1:-1]))))

  t2rdd = rdd2.groupByKey().map(lambda x: list(x[1])).filter(lambda x: len(x) > filter_val)

  return t2rdd

t2rdd = process_ta_feng_data(input_file, filter_val)
...
partitions2 = t2rdd.getNumPartitions()

t2_candidates = t2rdd.mapPartitions(lambda chunks : generate_candidates(chunks, support2, partitions2))
t2_candidates = t2_candidates.map(lambda x : (tuple(sorted(x)), 1)).reduceByKey(lambda y, z : z).keys().collect()

t2_candidates_sorted = sorted(t2_candidates, key=(lambda x: (len(x), x)))



t2_frequent_itemsets = t2rdd.mapPartitions(lambda chunks : generate_frequent_itemsets(chunks, t2_candidates_sorted)).reduceByKey(lambda x, y : x + y).filter(lambda z : z[1] >= support2).keys().collect()

t2_sorted_frequent_items = sorted(t2_frequent_itemsets, key = (lambda x: (len(x), x)))

def generate_results(output_file):
  with open(output_file, 'w') as f:
    f.write('Candidates:' + '\n')
    output_format(f, t2_candidates_sorted)

    f.write('Frequent Itemsets:' + '\n')
    output_format(f, t2_sorted_frequent_items)
